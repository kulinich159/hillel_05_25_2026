...
car_data_list = list(car_data.items())
car_data_list.sort(key=lambda item: item[1][4])
counter = 1

for brand, auto_criteria in car_data_list:
  if auto_criteria[1] >= year and auto_criteria[2] >= engine_volume and auto_criteria[4] <= price and counter <=5:
    counter += 1
    print(brand, auto_criteria)